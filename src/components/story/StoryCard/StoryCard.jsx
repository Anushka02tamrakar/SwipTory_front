...
const Story = ({ story, category }) => {
    ...
    if (category && typeof category === "string") {
        const foundSlide = story.slides.find(
            (element) => element.category === category
        );
        if (foundSlide) {
            storyImg = foundSlide.imageUrl;
        }
    } else {
        storyImg = story.slides[0].imageUrl;
    }

    // FOR THE FRONT HEADING
    let storyHeading;
    if (category && typeof category === "string") {
        const foundSlide = story.slides.find(
            (element) => element.category === category
        ...
        if (foundSlide) {
            storyHeading = foundSlide.heading;
        }
    } else {
        storyHeading = story.slides[0].heading;
    }
    // FOR THE FRONT DESCRIPTION
    ...
    if (category && typeof category === "string") {
        ...
        if (foundSlide) {
            storyDescription = foundSlide.description;
        }
    } else {
            ...
            <div className={styles.storyCard}>
                ...
                >
                    <div className={styles.storyDes}>
                        <h3 className={styles.storyTitle}>{storyHeading}</h3>
                        <p className={styles.storyContent}>{storyDescription}</p>
                    </div>
                </div>
