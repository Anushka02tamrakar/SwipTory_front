...
const Stories = ({ category }) => {
    ...
    const renderStories = (storyArray, isLoading, key, pageFunction) => (
        <>
            <div
                className={`${styles.stories} ${isSmallScreen ? styles.stories_mob : ""
                    }`}
            >
                {storyArray &&
                    storyArray.map((story) =>
                        isLoading ? (
                            ...
                            <Story key={story._id} story={story} category={key} />
                        )
                    )}
            </div>
        ...
        <div className={styles.storiesContainer}>
                            ...
                            (key) =>
                                stories[key].length > 0 && (
                                    <div key={key}>
                                        ...
                                        {renderStories(stories[key], storiesLoading, key, () =>
            ...
            {category !== "All" && (
                <div>
                    ...
                    {renderStories(categoryStories, categoryLoading, category, () =>
                        dispatch(getStoriesByCategory(category, page + 1))
                    )}
                    {categoryStories.length <= 0 && (
                        <h1 className={styles.no_story}>No stories Available</h1>
                    )}
                </div>
            )}
        </div>
